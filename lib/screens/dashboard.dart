...
class _DashBoardState extends State<DashBoard> {
  ...
  getDate() async {
    ...
    setState(() {
      date = DateFormat('MM/dd/yyyy')
          .format(now.toUtc().subtract(Duration(hours: 7)));
    });
  }
  ...
  calculateDistance({double sLat, double sLong}) async {
    ...
    distance = Geolocator.distanceBetween(
        sLat, sLong, double.parse(lat), double.parse(long));
  }
  ...
  onLogoutPressed(String note) async {
    ...
    try {
      ...
      var sub2 = await FirebaseFirestore.instance
          .collection('logs')
          .doc(widget.email)
          .collection('logs')
      ...
      calculateDistance(
          sLat: double.parse(locations[0]['loginLat']),
          sLong: double.parse(locations[0]['loginLong']));
      print('distance is' + distance.toString());
      ...
      if (distance < 76) {
        ...
        ToastBar(
                color: Colors.red,
                text:
                    'You must within the range of 250ft from your logged in location!')
            .show();
      }
    } catch (e) {
  ...
  notificationLogOut({String timestamp, String locationF}) async {
    DateTime now = await NTP.now();
    String time =
        DateFormat('hh:mm a').format(now.toUtc().subtract(Duration(hours: 7)));
    var durInMins = now
        .toUtc()
        .subtract(Duration(hours: 7))
        .difference(DateTime.parse(timestamp))
    ...
    var s = await FirebaseFirestore.instance
        ...
        .doc(widget.companyEmail)
        .collection('sites')
        .where('site', isEqualTo: locationF)
        .limit(1)
        .get();
    var locs = s.docs;
                  ...
                  Padding(
                    ...
                    child: Button(
                        ...
                        onclick: () async {
                          ...
                          await pr.hide();
                          ToastBar(text: 'Data updated!', color: Colors.green)
                              .show();
                          Navigator.pop(context);
                      ...
                      Padding(
                        ...
                        child: Container(
                          ...
                          child: Padding(
                            padding: EdgeInsets.all(ScreenUtil().setHeight(20)),
                            child: CustomText(
                              text: (double.parse(lat)).toStringAsFixed(5),
                              color: Colors.black,
                      ...
                      Padding(
                        ...
                        child: Container(
                          ...
                          child: Padding(
                            padding: EdgeInsets.all(ScreenUtil().setHeight(20)),
                            child: CustomText(
                              text: (double.parse(long)).toStringAsFixed(5),
                              color: Colors.black,
                              size: ScreenUtil().setSp(28),
                            ),
                          ),
                        ...
                        children: [
                          ...
                          Expanded(
                            child: SizedBox(
                              ...
                              text: location != null
                                  ? location
                                  : 'You are not within the work site!',
                  ...
                  showDialog(
                      ...
                      builder: (BuildContext context) {
                        return AlertDialog(
                          content: CustomText(
                            text:
                                'Are you sure you want to ${!logged ? 'log in' : 'log out'}?',
                            color: Colors.black,
                          ),
                          actions: [
                            FlatButton(
                                onPressed: () async {
                                  ...
                                  if (isLocationServiceEnabled) {
                                    ...
                                    if (permission ==
                                      ...
                                      ToastBar(
                                              text:
                                                  'Please accept location permissions for continue',
                                              color: Colors.red)
                                          .show();
                                      ...
                                      if (location != null) {
                                        if (!logged) {
                                          onLoginPressed();
                                          print(
                                              'login button pressed entering to login');
                                          Navigator.pop(context);
                                        } else {
                                          Navigator.pop(context);
                                          isForeman
                                    ...
                                    ToastBar(
                                            text:
                                                'Please enable location service for continue',
                                            color: Colors.red)
                                        .show();
            ...
            Visibility(
              ...
              child: Center(
                child: Padding(
                  ...
                  child: CustomText(
                    text: 'Last logged time - $lastTime',
                    size: ScreenUtil().setSp(35),
                  ),
                ),
              ),
